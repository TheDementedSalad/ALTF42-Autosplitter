...
init
{
	...
	if (updateProgressBar != 0)
	{
		...
		if (foundBytes.Length == goodBytes.Length)
		{
            ...
            {
				allocated += 0x200;
				ulong updateData = (ulong)vars.Helper.Scan(-14, "48 8B D6 FF 90 ?? ?? 00 00 84 C0 75 ?? 48 8B 4B 48");
				if (updateData != 0)
                {
					byte[] goodBytes2 = { 0x48, 0x8B, 0x43, 0xC8, 0x48, 0x8D, 0x4B, 0xC8, 0x45, 0x33, 0xC9, 0x4C, 0x8B, 0xC7 };
					...
					if (goodBytes2.SequenceEqual(foundBytes2))
					{
						byte[] s1 = { 0xFF, 0x25, 0x00, 0x00, 0x00, 0x00 };
						byte[] s2 = BitConverter.GetBytes((ulong)allocated);
						byte[] start = s1.Concat(s2).ToArray();

						byte[] e1 = { 0x51, 0x48, 0x31, 0xC9, 0x48, 0x89, 0x0D, 0xF5, 0xFE, 0xFF, 0xFF, 0x59 };
						byte[] e2 = goodBytes2;
						byte[] e3 = { 0xFF, 0x25, 0x00, 0x00, 0x00, 0x00 };
